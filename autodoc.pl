...
for (sort keys %funcflags) {
    ...
    #use Data::Dumper;
    #print STDERR __FILE__, Dumper \%funcflags{$_};
    warn "no docs for $_\n"
}
...
# experimental nor deprecated, nor beyond deprecated
my @missing_api = grep $funcflags{$_}{flags} =~ /A/
                    && $funcflags{$_}{flags} !~ /[xD]/
                    && ($funcflags{$_}{flags} !~ /b]/ || $funcflags{$_}{flags} =~ /Mm]/)
                    && !$docs{api}{$_}, keys %funcflags;
# If b and not m then is beyond deprecated.
# So if isn't b or is m
...
|specialized items are instead documented in L<perlapio>, L<perlmroapi>,
|L<perlcall>, L<perlclib>, L<perlguts>, L<perlxs>, L<perlfilter>, and
|F<config.h>.
|
...
|=head1 SEE ALSO
|
|L<perlguts>, L<perlxs>, L<perlxstut>, L<perlintern>, F<config.h> XXX
|
_EOE_
...
|=head1 SEE ALSO
|
|L<perlapio>, L<perlmroapi>,
|L<perlcall>, L<perlclib> L<warnings>, L<perlguts>, L<perlxs>,
|L<perlfilter>
|
END
