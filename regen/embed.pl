...
#print STDERR __LINE__, Dumper $embed, $core, $ext, $api;
...
# generate proto.h
{
	...
	die_at_end "For '$plain_func', M flag requires p flag" if $flags =~ /M/ && $flags !~ /p/;
	die_at_end "For '$plain_func', b and m flags are mutually exclusive (try M flag)" if $flags =~ /b/ && $flags =~ /m/;
	die_at_end "For '$plain_func', b flag without M flag requires D flag" if $flags =~ /b/ && $flags !~ /M/ && $flags !~ /D/;
